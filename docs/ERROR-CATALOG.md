...
| `CONNECTION_ERROR` | `ConnectionError` | Driver-level connection failure -- a network code, a Postgres class-08 / shutdown SQLSTATE, or a `28xxx` authentication failure (see the retryability note below: bound retries for a persistent auth/config failure). | retryable |
...
**Known limitation: a `28xxx` authentication failure surfaces as a retryable `ConnectionError`.**
`translatePostgresError` maps SQLSTATE `28000` (invalid_authorization_specification) / `28P01`
(invalid_password) to `ConnectionError`, which is marked **retryable**. A *transient* authorization
hiccup (a credential rotation racing a reconnect, a momentarily-unavailable auth backend) can clear
on an immediate retry. But a **persistent** auth/config failure -- a genuinely wrong password or a
revoked role -- will **not** clear on retry without a deployment-configuration change, so a caller
that retries a retryable `ConnectionError` unboundedly would spin. Callers MUST therefore **bound
their retries** (a capped attempt count / backoff ceiling) rather than retry a `ConnectionError`
indefinitely. A distinct **non-retryable** `AuthenticationError` code that splits `28xxx` off the
transient class-08 connection-loss codes is a candidate for an **additive 1.1 minor**: adding a new
error code is non-breaking under the [stability policy](STABILITY.md) (no existing `code` is removed,
renamed, or repurposed), so it is deliberately **out of the frozen 1.0 surface** rather than shipped
as a freeze-scope change.
...
its bound because another instance is **mid-migration**. The acquire is bounded by a `SET LOCAL
lock_timeout`, and `runMigrations` translates **both** SQLSTATEs that can terminate that bounded wait
into this one error: `55P03` (`lock_not_available` -- the `lock_timeout` cancellation) **and** `57014`
(`query_canceled` -- a shorter `statement_timeout` firing first during the same acquire). This is
**not** a conflation of two unrelated failures: both are, specifically, *the migration-lock acquire
timing out* -- they are caught only at the single `pg_advisory_lock` acquire site in
`src/postgres/migrate.ts` (via `isLockTimeout(err) || isStatementTimeout(err)`), never on a data-path
statement -- and both clear the same way: the lock is released the moment the concurrent migration
commits, so a bounded backoff-then-retry can succeed once that migration finishes. That is exactly
the same transient advisory-lock-wait character as the retryable `LEASE_TIMEOUT` (the writer-lease
acquire). An earlier draft marked it non-retryable on the grounds that "the orchestrator should back
off and restart the process"; the audit found that an *operational preference* for how to retry is
not a *retryability fact* about the error. The frozen retryable set is therefore
