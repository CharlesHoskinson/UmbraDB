...
| §5 Commit/transaction layer and writer lease | `sql.begin()` transactions, `sql.reserve()`-pinned advisory-lock writer lease | Drafted as `sprint-2-transaction-lease` (proposal/design/tasks/spec written, under Opus panel + Codex review as of this writing) — **not yet implemented**. |
