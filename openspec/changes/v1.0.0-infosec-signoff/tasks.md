...
- [x] 0.1 Write `SECURITY.md` at the repo root and link it from `README.md` (`design.md` §1).
...
- [x] 0.2 In `SECURITY.md`, state that `schema` is namespacing, NOT a security/tenant boundary,
...
- [x] 0.3 In `SECURITY.md`, state the global chunk-pool trust-domain requirement and the
...
- [x] 0.4 In `SECURITY.md`, state the **binding** no-at-rest-encryption precondition: secret-bearing
...
- [x] 0.5 In `SECURITY.md`, add the commit-policy section ("no key with any value in git"; the one
...
- [x] 0.6 In `SECURITY.md`, add the scope disclaimer naming the P1 fast-follows this doc documents
...
- [x] 1.1 Rewrite the three advertised-dedup doc sites in `src/interfaces/checkpoint-store.ts`
...
- [x] 2.1 Create `nix/midnight-env/README.md` (none exists today — only `test-wallets/README.md`)
...
- [x] 2.2 De-stub the VerifyFull/`--ca` path in `nix/midnight-env/scripts/enable-db-sync-tls.sh`:
...
- [x] 2.3 Verify the default (no-`--ca`) path provisions the **same** self-signed cert parameters,
...
- [x] 3.1 Add a repo-root `.npmrc` with `ignore-scripts=true`, and add a `.github/workflows/
...
- [x] 3.2 Add the blocking `npm audit --audit-level=high --omit=dev` step (plus an optional
...
- [x] 3.3 Add the `trivy` image scan of BOTH digest-pinned images (indexer-standalone
...
- [x] 3.4 Add the `flake.lock` **change-control** check to `supply-chain.yml`: compare the PR's
...
- [x] 3.5 Add the **full-history** gitleaks secret-scan step (`gitleaks detect`) to
...
- [x] 4.1 Untrack the live secret and add the allowlist + template + generator: `git rm --cached
  ...
  not).
- [x] 4.2 Strengthen `nix/midnight-env/test-wallets/README.md` with the generator/`.example`
...
- [x] 5.1 Whole-change differential review: an auditor re-reads this `proposal.md`/`design.md`/
  ...
  imported.
- [x] 5.2 Update `ROADMAP.md` §E to mark G15–G19 complete (and cross-reference that the
