...
- [ ] 0.2 **HOT-update regression test, direct SQL level.** Insert a `watermarks` row, then
  `UPDATE` it in place (same `kind`/`key`, different `value`) many times (e.g. 50) in a loop, and
  assert via `pg_stat_user_tables` (`n_tup_hot_upd` vs `n_tup_upd` for this table) that the ratio
  is at or near 1 — confirming the `fillfactor=90` setting actually delivers HOT updates in
  practice, not just that the reloption is present. **Acceptance:** the test reads
  `pg_stat_user_tables` before and after the update loop and asserts `n_tup_hot_upd` accounts for
  effectively all of the delta in `n_tup_upd` (allow a small tolerance for the very first insert,
  which is never a HOT update by definition).
...
- [ ] 1.2 **Non-object JSON root test** (`design.md` §2's `sql.json()` requirement, the spec's
  ...
  entire `value`, and confirm `get` returns each exactly. **Acceptance:** both cases pass without
...
- [ ] 3.2 **Connection-failure translation test** (`design.md` §8): force a connection failure
  ...
  `postgres.js` error. **Acceptance:** no new SQLSTATE translation is added to `src/postgres/
...
- [ ] 4.1 P9 (Law W1): `fast-check` property — `get` after N random `set`s to one key returns the
  ...
  not just one, matching `Formal/STORAGE_ALGEBRA.md` §5's own two-part wording for P9.
...
- [ ] 5.2 Update `ROADMAP.md`'s Milestone 2 checklist (mark Watermarks done — this completes
  Milestone 2 entirely) and `design/tasks.md`'s phase-map table (mark the §4 row superseded) so
  the roadmap doesn't drift from what's actually been built.
